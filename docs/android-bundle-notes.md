...
The Android dashboard passes `-1` for Off and sends indices 0 through 4 for visible levels 1 through 5. Selecting a non-Off bounce or sound level also turns that channel on; selecting Off turns only that channel off. Homey follows the same model with Off/1–5 pickers. Maximum bounce and sound remain owned by the native app; Homey reports them as read-only percentage sensors and can react to their Flow change triggers, but cannot change them. Its main On control reuses the saved picker selections and falls back to level 1 for both channels only when both selections are Off, ensuring On always starts soothing. All controls return after one accepted shadow update instead of polling for a second confirmation. The controller keeps one correlated response subscription for its connection, and Homey coalesces rapid level changes to the newest pending value rather than replaying every intermediate selection.
