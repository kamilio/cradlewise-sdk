...
Control methods return `CradleControlResult`, a partial state containing the acknowledged command changes and valid reported fields supplied by that response. Missing fields are unknown: consumers must preserve their existing observations rather than replace them with off/zero defaults. A lock-only acknowledgement without reported telemetry therefore returns lock fields, not fabricated bounce or sound status. Previous reads and optimistic commands are not carried forward as fresh observations. `active` is included only when either channel is known on or both are known off. A successful acknowledgement confirms the desired update, not that the hardware has applied it; use `getState()` or REST polling for a separate reported-state read. `getState()` retains its existing `CradleControlState` return contract.
